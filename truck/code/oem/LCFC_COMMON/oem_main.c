...
void Hook_Timer5msEvent(void)
{
  ...
  if (Eject_Enable()){
    if ((Target_BAT_state == BAT_SEL_S) && (BAT_Eject_Cnt == 0))
	 {
      //if(BAT_Eject_Cnt == 0){ 
        StopChgIC();
        BAT_Eject_Cnt ++;
...
//second battery
	...
	#if Support_TYPE_C
	//if(Read_EC_ON())  //A485D000132-: Add PD control for i2c retry--ti spec
	if(Read_RSMRST())  //A485D000132+
  	...
  	if (BattExistGet && !PriBattInhibGET )
  	{
    	if( IS_MASK_SET(uProject0,uFUdayClr) )
    	{
      		...
      		{  //A485D000149+S
                    ...
                    {//A485D000149+E
                        if((0x00 == uSMBusData0) && (0x00 == uSMBusData1))
                        {
...
void Hook_Timer100msEventB(void)
{
  ...
  if (SecBattExistGet && !SecBattInhibGET )
  {
    if( IS_MASK_SET(uBattFUDStatus,uFUdayClr2) )
    {
      ...
      {//A485D000149+S
        if(bRWSMBus(chSMB_2ndBAT, SMbusRW, SecSmBat_Addr, FirstUsedDate, &uSMBusData0,SMBus_NeedPEC))
        {
            if((0x00 == uSMBusData0) && (0x00 == uSMBusData1))
            {//A485D000149+ E
...
//#if Thinkpad_Battery_HW_Auth
  	...
  	{
    	if (SecBattExistGet && (1 == S_BATdata_ready)) // Add battery HW_Auth condition
...
//#endif  // Thinkpad_Battery_HW_Auth
    ...
    OEM_Hook_Timer500msEventA();
			...
			{
			   FullCHG_SetChargerMinSystemVoltage(); //A485D000124: Fix battery can not charge full in S0
                #if 0 // for test
					if(IS_MASK_SET(SYS_STATUS,AC_ADP) )
					{
...
//Fix P2P function abnormal A485D00084
	...
	//A485D00080--->S
 //if(IS_MASK_SET(SYS_STATUS,AC_ADP)&&(CurrentTypeCWatt==0))//&&(CurrentRDO == ObjectPosition20V))
//	{
//	  ReadAdaIDFailCnt ++;
//	  if(ReadAdaIDFailCnt>4)
	 // { 
	
	 //  Send4ccCmd(0x0B,TYPECPort1);       
	 //  Send4ccCmd(0x0B,TYPECPort2);
//	  }
//	 }
//	 else
//	 {ReadAdaIDFailCnt=0;}
	...
	if(thermalTBlMode == PFM_Mode) //A485D00091
		{
          ...
          // #endif //Support keyboard backlight detect
                if(SystemIsS0) //A485D000111: fix backlight led blink breath as lenovo spec
	        {
	        	KBL_HotKey_Blink_Achive = KBL_BLICK_TIMES_HOTKEY_PERFORMANCE;
