...
{
  BYTE Temp8;
  BATTBATLOWSHDProt = FALSE;		// //A485D00077+
  ...
  u4SenseSTS3 &= ~(BIT3+BIT4); //A485D000145+:
  EC_ON_HI();
...
void AdapterOut(void)
{
  ...
  Port2UnknowAdpRetry=0;//A485D000145:+
  UnknowAdpRetry=0;//A485D000145:+
  TypeC_LenovoAdaptor &= 0xFC; //A485D000151+
...
//  if (uReserve07.fbit.nPmeWakeEN==0) return;
  ...
  switch (SysPowState)
  {
    ...
    case SYSTEM_S5 :
      ...
      if (WOLACWakeEnGet || WOLDCWakeEnGet) {	 // A485D00075+
        PWSeqStep = 1;
        PowSeqDelay = 0x00;
        SysPowState=SYSTEM_S5_S0;
      }
	...
	switch(device_id)
      ...
      break;      
    	...
    	case 4 :
				...
				#else
				#if  Support_EC_LANWake
      			//new_state = (Read_LAN_WAKE_IN()||Read_SYS_LAN_WAKE_IN()); //A485D00073-// A485D00031:Enable AC/DC wake on Lan function
                  new_state = Read_SYS_LAN_WAKE_IN();//A485D00073+
				#else
      			new_state = 0x00;
				#endif  // Support_EC_LANWake
				#endif		// End of EventDebug
