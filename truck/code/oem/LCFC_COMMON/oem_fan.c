  ...
  else
  {
      switch( TmlICStep & 0x03 )
      {
        ...
        case 1:
          ...
          break;
        case 2:
	   nCpuTempLocal = TEMP_TS_2; //A485D00092
	   TEMP_TS_2 = TEMP_TS_1;
	   TEMP_TS_1 = BsData;
	   // 3s calculate per times - 20180402
...
void SBTSI_ReadCPUTemp(void)
{
  ...
  else
  {
    ...
    if ( IS_MASK_CLEAR(Fan_Debug_Temp,b0CPU_Temp) )  // Debug CPU Temperature, Engineer myself control.
    {
     ...
     //5s read calculate per times  - 20180402
...
void Oem_Fan_Control(void)
{
  ...
  //A485D00039---start
	...
	#endif	// end of Support_IFC//A485D00039---end
...
void FanCtrlWin(void)
{
	BYTE BCPULevel,Level_chg, BTSLevel;
  	#if Support_VGA_FANTABLE  //A485D00039
  	BYTE	BVGALevel;
  	#endif	// end of Support_VGA_FANTABLE
  ...
  VFAN_Tab_DAC  = FanTblPtr[FAN_ID_TYPE].VgaTbl[BVGALevel].RPMValue;
	#endif	// end of Support_VGA_FANTABLE  //A485D00039
  ...
  else
  {
	...
	if(FANEXTSeg == 1)
	{
		if(TargetRPM < STP_FAN_RPM)
		{
			...
			else if(FAN_ID_TYPE == FAN_VENDOR_3)
...
void ThrottlingWin(void)
{
  ...
  if ( nCpuTempLocal >= TurboOff_External_Sensor)  // CPU turbo turn OFF.
	...
	if(nCpuTempLocal >= STP_SHDN)
		Thro_Count_Shut(&Ext_CPU_Shut_Cnt,TEMP_TYPE_EXT);
...
void Thro_Count_Shut(BYTE *BShut_Cnt, BYTE TempType)
{
  if ( *BShut_Cnt >= Thro_Shut_Cnt )  // Read 3 times.
  {
	...
	{ ProcessSID(EXTCPUOVERTEMP_ID); }  // Ext CPU Shutdown ID 0x12. //A485D00039+		
    else
