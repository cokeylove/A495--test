...
//  SET_MASK(CHGIC_ReadCmd0x31L,IFAULT_LOW);
  ...
  CHGIC_ReadTempH = 0x00;//A485D000103115 //0x02; //A485D000103
  CHGIC_ReadTempL = 0x00;//A485D000103115//0xC0;  //A485D000103
  ...
  ChargeIC_InitCnt = CHARGE_IC_VOLTAGE_SET_COUNT; //A485D000103115 set time out 3s
}
...
void WriteSmartChgIC(void)
{
  ...
  switch( CHGIC_ptr )
	...
	//A485D000103+ for charger test
	 bRWSMBus(chSMB_CHG, SMbusRW, Charger_Addr, C_InputVoltage, &CHGIC_ReadTempL, SMBus_NoPEC);
            CHGIC_ReadTempH = 0x00; //A485D000115 //0x02; //A485D000103
            CHGIC_ReadTempL = 0x00; //A485D000115 //0xC0; //A485D000103
    if (!bRWSMBus(chSMB_CHG, SMbusWW, Charger_Addr, C_InputVoltage, &CHGIC_ReadTempL,SMBus_NoPEC))
    {
      CHGIC_SMbusFailCnt++;
     }
...
{
  if (OnOff == Enable){     
    WSMbusTemp02=0x31;      //12.95V //A485D000117
    WSMbusTemp01=0x00;      //A485D000117
  }else{  //A485D000124: Fix battery can not charge full in S0
    if(Chk_AllBattery_Full()){	
	//A485D000144--->Start
     ...
     else if(Current_BAT_State == BAT_SEL_S)
      {
  	  ...
  	  ITempW01 = 10800;  // return;
	 
	 if((ITempW01 >= 11000) && (ITempW01 <= 12000)){
	 
       WSMbusTemp02=0x34;  //12.8V //A485D000126:
       WSMbusTemp01=0x00; 
	 }
	else if((ITempW01 >= 10000) && (ITempW01 <= 11000)){
		
       WSMbusTemp02=0x32;  //12.8V //A485D000126:
       WSMbusTemp01=0x00; 
	 }
...
//A485D000124: Fix battery can not charge full in S0 
void FullCHG_SetChargerMinSystemVoltage(void)
{    
   if(Chk_AllBattery_Full()) 
   	{  	//A485D000144--->Start
	   if (Current_BAT_State == BAT_SEL_M)
	 ...
	 if((ITempW01 >= 11000) && (ITempW01 <= 12000)){
       ...
       WSMbusTemp02=0x34;  //13.3V //A485D000126:
       WSMbusTemp01=0x00; 
	   MinSysVolConfirmByte=0x34;
	 }
	else if((ITempW01 >= 10000) && (ITempW01 <= 11000)){
		
       WSMbusTemp02=0x32;  //12.5V //A485D000126:
       WSMbusTemp01=0x00; 
	   MinSysVolConfirmByte=0x32; 
	 }	
	  
	 if(Read_CHIC_MinSystemVoltage2 != MinSysVolConfirmByte)  //A485D000126	//A485D000144--->End
   	  { 
	  if (!bRWSMBus(chSMB_CHG, SMbusWW, Charger_Addr, C_MinSystemVoltage, &WSMbusTemp01,SMBus_NoPEC))
	    {
		  CHGIC_SMbusFailCnt++;
	    ...
	    {
		 CHGIC_SMbusFailCnt = 0;
         bRWSMBus(chSMB_CHG, SMbusRW, Charger_Addr, C_MinSystemVoltage, &Read_CHIC_MinSystemVoltage1,SMBus_NoPEC);
	    }    
 	 }
    }   
   else  if(Read_CHIC_MinSystemVoltage2 != 0x24)
   	{
          WSMbusTemp02=0x24;  //A485D000126:
	   WSMbusTemp01=0x00; 
	  if (!bRWSMBus(chSMB_CHG, SMbusWW, Charger_Addr, C_MinSystemVoltage, &WSMbusTemp01,SMBus_NoPEC))
	    {
		  CHGIC_SMbusFailCnt++;
