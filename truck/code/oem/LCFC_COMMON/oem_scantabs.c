...
//  HotKey_5,           // Hotkey5
  ...
  HotKey_5,           // Hotkey5 //A485D00035:Add Fn + Q send Q event
};
...
{               //  Normal     Fn         // Keyboard matrix index
  ...
  0x15, 0xE2,   //  Q          Q Event(0x28)  //0xF4 ////A485D00035:Add Fn + Q send Q event--->
#endif //CSs16KB  
...
// Function key F1 ~ F12
  ...
  18  , DO_COSTOMER_FUNCTION,     // Function key Q    [E2h] //A485D00035:Add Fn + Q send Q event--->
};
                  // Reserved 0xE2 ~ 0xFF
...
void Hook_keyboard_Oem(uKEY *key,  BYTE event)
{
  if( FnCtrlSWAPGet )
  {
    if((key->field.input== 0x00)&&(key->field.output == 0x0F))// LCtrl //A485D00045
    {
      // Replaced by Fn key
      key->field.input = 0x04;
      key->field.output = 0x0C; // Fn
    } else if((key->field.input == 0x04)&&(key->field.output == 0x0C)) { // Fn
      // Replaced by LCtrl key
      key->field.input = 0x00;   //A485D00045
      key->field.output = 0x0F; // LCtrl  //A485D00045
    }
  }
