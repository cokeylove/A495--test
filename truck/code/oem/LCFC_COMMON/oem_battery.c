...
//trickle charge setting
  ...
  if( IS_MASK_SET(u4PeriSTS5,(PriBattDisChg << battery_id)))//A485D000121 
  {
...
// A485D000127
	  if(IS_MASK_SET(OEM_BAT(M_BATAlarm, battery_id),BATOCP) && IS_MASK_CLEAR(GPU_THROTTLE_SRC, BAT_OCP_Event_En))
	  	{
	  	  SET_MASK(GPU_THROTTLE_SRC, BAT_OCP_Event_En);
          ECSMI_SCIEvent(CPU_Set12W_enable);
	    }
	  else if(IS_MASK_CLEAR(OEM_BAT(M_BATAlarm, battery_id),BATOCP) && IS_MASK_SET(GPU_THROTTLE_SRC, BAT_OCP_Event_En))
	  	   {
             CLEAR_MASK(GPU_THROTTLE_SRC, BAT_OCP_Event_En);
             ECSMI_SCIEvent(CPU_Set12W_Disable);
	       }
 //A485D000127	  
...
void Calculate_AVTimeEmpty(int battery_id, BYTE *Data){
  DWORD Temp32;// AVGcurrent, AVGvolatage;
  Temp32 = (TWR_BAT(RemainCapH, battery_id) << 8) + TWR_BAT(RemainCapL, battery_id);
  Temp32 &= 0x0000FFFF; //A485D00074+
  AVGcurrent = 0xFFFF - ((TWR_BAT(AvgCurrentH, (1^battery_id)) << 8) + TWR_BAT(AvgCurrentL, (1^battery_id)));  
  AVGvolatage = (TWR_BAT(VoltageH, battery_id) << 8) + TWR_BAT(VoltageL, battery_id); 
  /*//A485D00070-: Fix battery low EC exception-->S.
 ...
 *///A485D00070-: Fix battery low EC exception-->E
   //A485D00070+: Fix battery low EC exception-->S
...
//A485D000118--->End	   
	 if ((ITempW01 > BAT3S4SVot_Divide)&&(ITempW01 < 20000))  ////A485D000118
...
//A485D000124: Fix battery can not charge full in S0
BYTE Chk_AllBattery_Full(void)
{
  if(IS_MASK_CLEAR(SYS_STATUS,AC_ADP))
  	 return FALSE;
  ...
  if( AllBattExistGet) 
  	{
  	 if(IS_MASK_SET(BattStatusL,FullyChg) && IS_MASK_SET(SecBattStatusL,FullyChg))
     return TRUE;
  	}
 else if (((BattExistGet) && IS_MASK_SET(BattStatusL,FullyChg)) 
	 	||((SecBattExistGet) && IS_MASK_SET(SecBattStatusL,FullyChg)))
 	   { return TRUE; }
...
//main battery
  if(BattExistGet
    || (IS_MASK_SET(EVT_STATUS1, M_bat_in) && (IS_MASK_SET(u4PeriCtrl1,Edge_PriBattInhib) )))//A485D000150+
 {
//    if(IS_MASK_SET(OptMfgFunction2L, BIT7)) // Replaceable(0)/Non-replaceable(1)
    {
       SET_MASK(uShipModeWorkRun, BAT1_SHIPMODE_ENABLE); //A485D000150+
       CLEAR_MASK(uShipModeWorkRun, BAT1_SHIPMODE_SUCCESS);//A485D000150+
...
//    if(IS_MASK_SET(SecOptMfgFunction2L, BIT7)) // Replaceable(0)/Non-replaceable(1)
    {
       SET_MASK(uShipModeWorkRun, BAT2_SHIPMODE_ENABLE);//A485D000150+
       CLEAR_MASK(uShipModeWorkRun, BAT2_SHIPMODE_SUCCESS);//A485D000150+
...
{
  WORD cutoff_volt,shutdnFlag;		// A485D00077+

  shutdnFlag = FALSE;				// A485D00077+
...
//#endif
  ...
  if ((ITempW01 <= cutoff_volt) || (shutdnFlag == TRUE))  //  shutdown Voltage //  A485D00077+
  {
    BatLowCnt++;
    if ( (BatLowCnt>=6)&&((SystemIsS0)||(SystemIsS3)) )
    {
    	BATTBATLOWSHDProt = TRUE;						//  A485D00077+
      SET_MASK(SysStatus,ERR_ShuntDownFlag);
...
void LV_BAT_Authentication(void)
{
 ...
 // if(((WORD)((TWR_BAT(VoltageH,S_BAT)<<8)+TWR_BAT(VoltageL,S_BAT)))< OEM_BAT(M_BatLowVoltage,S_BAT))  //A485D00017-//A485D00079+
  ...
  if(((WORD)((TWR_BAT(VoltageH,S_BAT)<<8)+TWR_BAT(VoltageL,S_BAT)))< //A485D00079-
 	((WORD)((OEM_BAT(M_BatLowVoltage_HB,S_BAT)<<8)+OEM_BAT(M_BatLowVoltage_LB,S_BAT)))) //A485D00095 A485D00017+//A485D00079-
  { return; }
  //===================================================================
...
//      SetRandomKey(1);    // initial W(4~8)
    ...
    case 25:
#if Thinkpad_Battery_HW_Auth
      //GetChallengeFromBat(); //A485D00076-
	  //A485D00076+  --->S
...
void BatteryFirstUseDay(void)
{
  if( IS_MASK_SET(uProject0,uSysTimeOK) )
  {
    ...
    if(( p2FUDayTemp <= 127 )&&(p2FUDayTemp >=1)) //A485D000134
    {
    ...
    if(( p2FUDayTemp <= 12 )&&(p2FUDayTemp >=1))//A485D000134
    {
    ...
    if(( p2FUDayTemp <= 31 ) && (p2FUDayTemp >= 1)) //A485D000134
    {
...
//(Rpc_L+Rpack_L*16)*(Rpc_H+Rpack_H*16)+0x339E)
  ...
  else if( IS_MASK_SET(OEM_BAT(M_BAT_uAuthenFlag,battery_id),uDF_Return) && (TEMPNpack == TEMPNpc) )
  {
    ...
    if(IS_MASK_SET(OEM_BAT_1B(PriBattSTS,battery_id),BattExist))
    {
      ...
      //A485D000123: fix battery auth success bios show error for status unsync.
...
void LAuthProcess(int battery_id)
{
 ...
 if(((WORD)((TWR_BAT(VoltageH,battery_id)<<8)+TWR_BAT(VoltageL,battery_id)))< 
 	((WORD)((OEM_BAT(M_BatLowVoltage_HB,battery_id)<<8)+OEM_BAT(M_BatLowVoltage_LB,battery_id))))//A485D00095//A485D00017+ //A485D00079-
  { return; }
...
void TPG_EXTBATCmd(void)
{
   switch(u4ExtCommand)
  {
    ...
    case 0x03:
		...
		u4Cfgspace4 &= ~u4Cfgspace4BATseq; //A485D00047: Fix Charge test fail
		CLEAR_MASK(ISCT_Again, PKShiftCMD);//A485D000121
        break;
    case 0x04:
		RamDebug(0xDB);
		RamDebug(0x04);
		...
		SET_MASK(ISCT_Again, PKShiftCMD);//A485D000121
        break;
    case 0x05:
...
void Switch_Battery_Process (void)
{
  ...
  if (BAT_switch_delay_timer >= BAT_switch_delay_time){  // A485D000131
    Switch_Battery();
...
BOOL Chk_Stop_charge_flag()
{
	...
	{			// BAT2 stop charge	
	   CLEAR_MASK(nBatteryStatH, SecBat_CHARGE );  //A485D000142
	 if(IS_MASK_CLEAR(BattStatusL,FullyChg)) //A485D000142 add for switch to 2nd Bat at both bat full charge 
	  {
	   BAT_state_target(BAT_SEL_M);									// select main battery     
       return TRUE;
	  }
       ...
       return;
		...
		{														// now is charging Battery1
			...
			else
			{
				...
				{												// Battery2 = 100%
					////A485D000142 add for switch to 2nd Bat at both bat full charge 
		...
		else if(Current_BAT_State == BAT_SEL_S)
		{
			...
			{
				if(IS_MASK_CLEAR(BattStatusL,FullyChg)) //A485D000142+ add for switch to 2nd bat at both batt full 
					BAT_state_target(BAT_SEL_M);								// select Main battery
			}
		}
		else
